...
module GenericFileHelper
  ...
  def render_download_icon(title = nil)
    if title.nil?
      link_to download_image_tag, sufia.download_path(@generic_file), title: "Download the document", id: "file_download", data: { label: @generic_file.id }, download: @generic_file.title.first+'.pdf'
    else
      label = download_image_tag(title)
      link_to label, sufia.download_path(@generic_file), title: title, id: "file_download", data: { label: @generic_file.id }, download: @generic_file.title.first+'.pdf'
    end
  end
