    ...
    if document.collection?
      ...
      if document["hasCollectionMember_ssim"].present?
        document["hasCollectionMember_ssim"].each do |member|
          ...
          if visibility_check.present? and visibility_check['is_public_ssi'] == 'true' and visibility_check['flagged_tesim'] != ['Explicit content in thumbnail'] and visibility_check['mime_type_tesim'].present? and visibility_check['mime_type_tesim'][0].include?('image')
            path = sufia.download_path member, file: 'thumbnail'
            options[:alt] = "#{visibility_check['mime_type_tesim']}"
