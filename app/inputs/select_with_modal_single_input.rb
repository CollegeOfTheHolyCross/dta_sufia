class SelectWithModalSingleInput < SingleCollectionValueBase
  include WithHelpIcon

  #FIXME:
  def buffer_each(collection)
    collection.each_with_object('').with_index do |(value, buffer), index|
      buffer << yield(value, index) unless @rendered_first_element
    end
  end
  ...
  def build_field(value, _index)
    ...
    html_options[:class] ||= []
    html_options[:class] += ["#{input_dom_id} form-control  form_single_style"]
