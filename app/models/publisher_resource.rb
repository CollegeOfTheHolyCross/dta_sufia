class PublisherResource
  ...
  def self.find_location(subject)
    ...
    solr_response = ActiveFedora::Base.find_with_conditions("publisher_ssim:#{solr_wildcard_clean(subject)}*", rows: '50', fl: 'publisher_ssim' )
    solr_response_tesim = ActiveFedora::Base.find_with_conditions("publisher_tesim:#{solr_exact_clean(subject)}", rows: '50', fl: 'publisher_tesim' )

    #FIXME - A result for "http" gives back the entire array of values...
    values = []
    if solr_response.present?
      solr_response.each do |indv_response|
        indv_response["publisher_ssim"].each do |indv_subj|
    ...
    if solr_response_tesim.present?
      solr_response_tesim.each do |indv_response|
        indv_response["publisher_tesim"].each do |indv_subj|
          if indv_subj.match(/#{subject}/)
            values << indv_subj
          end
        end
    ...
    if values.present?
      ...
      return values.map! { |item|
        ##{URI.escape(item)}
        count = ActiveFedora::Base.find_with_conditions("publisher_ssim:#{solr_exact_clean(item)}", rows: '100', fl: 'publisher_ssim' ).length
        if count >= 99
          count = "99+"
        else
          count = count.to_s
