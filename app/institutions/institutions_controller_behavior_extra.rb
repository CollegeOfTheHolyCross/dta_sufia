module InstitutionsControllerBehaviorExtra
  extend ActiveSupport::Concern

  include Blacklight::Base
  include SelectsInstitutions

  included do
    include Hydra::Collections::AcceptsBatches #FIXME
  ...
  def after_create
    respond_to do |format|
      ...
      format.html { redirect_to collections.collection_path(@collection), notice: 'Institution was successfully created.' }
  ...
  def after_destroy (id)
    respond_to do |format|
      format.html { redirect_to catalog_index_path, notice: 'Institution was successfully deleted.' }
  ...
  def after_destroy_error (id)
    respond_to do |format|
      format.html { redirect_to catalog_index_path, notice: 'Institution could not be deleted.' }
  ...
  def move_members_between_collections
    destination_collection = ::Institution.find(params[:destination_collection_id])
    remove_members_from_collection
    add_members_to_collection(destination_collection)
    if destination_collection.save
      flash[:notice] = "Successfully moved #{batch.count} files to #{destination_collection.title} Institution."
    else
      flash[:error] = "An error occured. Files were not moved to #{destination_collection.title} Institution."
    end
  end
