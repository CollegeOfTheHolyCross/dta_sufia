class InstitutionSearchBuilder < Hydra::SearchBuilder
  def collection
    scope.collection
  end
...
# include filters into the query to only include the collection memebers
  ...
  def add_collection_filter(solr_parameters)
    solr_parameters[:fq] ||= []
    solr_parameters[:fq] << ActiveFedora::SolrQueryBuilder.construct_query_for_rel(has_model: ::Institution.to_class_uri)
  end
