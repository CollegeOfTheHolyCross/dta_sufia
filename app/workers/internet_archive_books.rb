...
class InternetArchiveBooks
  ...
  def perform(upload_collection_id, upload_institution_id, depositor)
    collection_id = 'digitaltransgenderarchive'
    @upload_collection_id = upload_collection_id
    @upload_institution_id = upload_institution_id

    @depositor = depositor
    #@url = "http://archive.org/advancedsearch.php?q=collection%3A%22#{collection_id}%22&fl%5B%5D=identifier&output=json&rows=10000"
    @url = "http://archive.org/advancedsearch.php?q=collection%3A%22#{collection_id}%22&fl%5B%5D=identifier&output=json&rows=50"
